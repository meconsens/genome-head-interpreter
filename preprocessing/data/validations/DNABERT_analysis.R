...
# DNABERT scores ----
## Preprocessing ----
# scores <- pd$read_pickle('DNABERT/enhancer_scores_dictionary_attention_updated_annotations.pkl')
...
# qsave(scores, file = "/DNABERT/enhancer_scores_dictionary_attention_updated_annotations_preprocessed.qs")

## Open scores ----
scores <- qread("/DNABERT/enhancer_scores_dictionary_attention_updated_annotations_preprocessed.qs")
...
coefficients <- ldply(fromJSON(file = "/DNABERT/coefficients/DNABERT.json"), extract_coefficients) %>% 
  as_tibble() %>% 
  bind_rows() %>% 
...
filter_coefficients <- coefficients %>% 
  ...
  dplyr::mutate(n = n()) %>% 
  dplyr::ungroup()
# write_csv(filter_coefficients, file = '/DNABERT/coefficients/dnabert_filter_coefficients.csv')
# id_correlations <- adply(filter_coefficients, 1, get_id_correlations, .progress = 'time') %>%
#   as_tibble() %>%
#   qsave(., file = '/DNABERT/coefficients/dnabert_individual_coefficients.qs')
# write_csv(id_correlations, file = '/DNABERT/coefficients/dnabert_individual_coefficients.csv')
id_names_map <- scores %>%
  dplyr::select(id,name) %>%
  dplyr::distinct()
id_correlations <- qread('/DNABERT/coefficients/dnabert_individual_coefficients.qs') %>% 
...
### layer1-head11 - SINE + LTR ----
quantile(scores$repeat_SINE[which(scores$repeat_SINE != 0)])
quantile(scores$repeat_DNA[which(scores$repeat_DNA != 0)])

scores %>% 
  dplyr::filter(name %in% c('sequence139', 'sequence235', 'sequence424', 'sequence434')) %>% 
  dplyr::filter(layer == 'layer1' & head == 'head11') %>% 
  dplyr::select(name, position, attention_scores, repeat_SINE, repeat_LTR) %>% 
  dplyr::group_by(name) %>% 
  dplyr::mutate(ymin = mean(attention_scores)) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(color = ifelse(repeat_SINE >= 1 & repeat_LTR >= 1,
                               "Overlap",
                               ifelse(repeat_SINE >= 1,
                                      ...
                                      ifelse(repeat_LTR >= 1,
                                             'Repeat LTR',
                                             "No Feature"))),
                color = factor(color, levels = c("No Feature", "Repeat SINE", "Repeat LTR", 'Overlap')),
                position2 = position+1,
                group = "group",
                name = factor(name, 
                              levels = c('sequence139', 'sequence235', 'sequence424', 'sequence434'),
                              labels = c('Sequence 139', 'Sequence 235', 'Sequence 424', 'Sequence 434'))) %>% 
...
#   dplyr::ungroup()
# write_csv(most_activated_kmers, file = "/DNABERT/kmer_analysis/most_activated_kmers.csv")
most_activated_kmers <- read_csv("/DNABERT/kmer_analysis/most_activated_kmers.csv")
...
#   write_csv(file = "/kmer_analysis/kmers_abundance_by_layer.csv")
...
#   dplyr::ungroup()
# write_csv(most_activated_kmers_analysis, file = "/DNABERT/kmer_analysis/most_activated_kmers_analysis.csv")
most_activated_kmers_analysis <- read_csv("/DNABERT/kmer_analysis/most_activated_kmers_analysis.csv")
...
most_activated_kmers %>% 
  ...
  l_ply(., function(y){
    ...
    ggsave(filename = glue::glue('/DNABERT/plots/kmer_analysis/logos/logo_{layern}.png'), plot = gg_final, bg = 'white')
})

## Genome comparison - Pretrained vs. Fine-tune ----
### Genome ----
kmers_abundance <-  read_delim('/DNABERT/kmer_analysis/genome_information/kmer_count_encode_all_enhancers_hg38.txt', show_col_types = FALSE) %>%
  dplyr::left_join(read_delim('/DNABERT/kmer_analysis/genome_information/kmer_count_repeat_masker_hg38.txt', show_col_types = FALSE), by = "kmer") %>%
  dplyr::left_join(read_delim('/DNABERT/kmer_analysis/genome_information/kmer_count_whole_genome_hg38.txt', show_col_types = FALSE), by = "kmer") %>%
  dplyr::rename(kmers = kmer, k.enhancer = count.x, k.repeat = count.y, k.genome = count) %>% 
  dplyr::mutate(p.enhancer = k.enhancer/sum(k.enhancer)*100,
                p.repeat = k.repeat/sum(k.repeat)*100,
                p.genome = k.genome/sum(k.genome)*100,
                pg.enhancer = p.enhancer/p.genome,
                pg.repeat = p.repeat/p.genome) %>% 
  dplyr::filter(str_detect(kmers, "N", negate = TRUE))
### DNABERT ----
dnabert_kmers <- bind_rows(read_csv("/DNABERT/kmer_analysis/most_activated_kmers.csv") %>% 
                             dplyr::mutate(file = "finetuned"),
                           read_csv("/DNABERT/kmer_analysis/most_activated_kmers_pretrained.csv") %>% 
                             dplyr::mutate(file = "pretrained"))
### Merge data ----
kmers_genome_df <- dnabert_kmers %>% 
  dplyr::select(-name, -position) %>% 
  dplyr::left_join(kmers_abundance %>% dplyr::select(kmers, pg.enhancer, pg.repeat), by = "kmers") %>% 
  dplyr::mutate(ratio = pg.enhancer/pg.repeat)
### Divide by enhancer or repeat enrich ----
kmers_genome_df %>% 
  dplyr::mutate(type = ifelse(pg.enhancer/pg.repeat >= 1.5,
                              "enhancer enriched",
                              ifelse(pg.repeat/pg.enhancer >= 1.5,
                                     "repeat enriched",
                                     "no_enrich"))) %>% 
  dplyr::group_by(layer, head, file, type) %>% 
  dplyr::summarise(n = n()) %>%
  dplyr::group_by(layer, head, file) %>% 
  dplyr::mutate(n = n/sum(n)*100) %>% 
  dplyr::ungroup() %>% 
  dplyr::filter(type != "no_enrich") %>% 
  dplyr::mutate(layer = factor(layer,
                               levels = paste0("layer", 11:0)),
                head = factor(head,
                              levels = paste0("head", 0:11)),
                file = factor(file, 
                              levels = c("pretrained", "finetuned"))) %>% 
  tidyr::pivot_wider(names_from = file,
                     values_from = n) %>% 
  dplyr::mutate(ratio = log10(finetuned/pretrained)) %>% 
  dplyr::select(-finetuned, -pretrained) %>% 
  ggplot(aes(x = head, y = layer, fill = ratio)) +
  facet_wrap(~type) +
  geom_tile() +
  scale_fill_gradientn(colors = rev(brewer.pal(11, "RdYlBu")),
                       breaks = seq(-6, 6, 3)/10,
                       labels = c("4x Pre-trained", "2x Pre-trained","No difference","2x Fine-tuned","4x Fine-tuned"),
                       limits = c(-0.7,0.7)) +
  labs(title = "Enhancer/Repeat regions kmer enrichment",
       subtitle = "DNABERT fine-tuning vs. pre-training",
       fill = "Ratio") +
  theme_minimal() +
  theme(axis.title = element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1),
