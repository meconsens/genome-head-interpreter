...
def parallel_shuffle_and_analysis(df, attention_score_columns, bio_feature_columns, seq_length, model, full_path, iteration):
    ...
    # save results
    os.makedirs(f'{full_path}/data/distributions/{model}-UNCENTERED/', exist_ok=True)
    results_filename = f'{full_path}/data/distributions/{model}-UNCENTERED/coef_{iteration}_results.csv'
